...
module.exports.updateComponent = function(component, config, username, callback){
    ...
    delete component._id;
    UpdatePkgKeyword(config, username)
        .then(
            function(){
        ...
        .done();
//    fis.db.update(db_component, username, {name : config.name}, component, {}, function(err, result){
//        if(err){
//            callback(err);
//        }else{
//            //todo 更新components的时候，修改pkgKeyword表
//            UpdatePkgKeyword(config, username)
//                .then(
//                    function(){
//                        callback(null);
...
function UpdatePkgKeyword(config, username){
    var defer = Q.defer();
...
module.exports.addComponent = function(config, username, callback){
    ...
    UpdatePkgKeyword(config, username)
        ...
        .fail(function(err){ callback(err); })
        .done();
//    fis.db.insert(db_component, username, config, {}, function(err, result){
//        if(err){
//            callback(err);
//        }else{
//            //todo 更新components的时候，修改pkgKeyword表
...
module.exports.getCategories = function(callback){
    //todo 所有组件的类别,以及数目从数据库中读取，现在直接返回all
    //todo fis.db 添加count接口
    fis.db.find(db_component, ROOT_USER, {}, {}, {}, function(err, cursor){
        if(err){
            callback(err);
        }else{
